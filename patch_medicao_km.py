...
# 1. Add kmsData state and modal state
state_block = """  const [isNovoPosto, setIsNovoPosto] = useState(false);
...
content = content.replace("  const [isNovoPosto, setIsNovoPosto] = useState(false);", state_block)

# 2. Add clientesNaFicha in medicaoProcessada
process_head = """    // 3. Agrupar presenças por codcli + codpos + turno
...
content = content.replace("""    // 3. Agrupar presenças por codcli + codpos + turno
    const mapaPresencas = new Map();
    presTrabalhadas.forEach(p => {""", process_head)

# 3. Update the mapping logic
map_logic_old = """      const diferenca = valorTotalReal - valorMensal;
...
map_logic_new = """      const diferenca = valorTotalReal - valorMensal;
...
# 4. Update the unmapped logic
unmap_logic_old = """          total_colaboradores: presInfo.colaboradores.size,
          valor_total: 0,
          diferenca_mensal: 0,
          detalhes: presInfo.detalhes,
          _nao_cadastrado: true  // Flag para identificar visualmente
        });"""
unmap_logic_new = """          total_colaboradores: presInfo.colaboradores.size,
          valor_total: 0,
          diferenca_mensal: 0,
          detalhes: presInfo.detalhes,
...
content = content.replace(unmap_logic_old, unmap_logic_new)

# 5. Add button "Lançar KM" near "Gerenciar Postos"
btn_old = """            <button 
...
# 6. Add "KM" and "Status" to Table Headers
th_old = """                  <th style={{ padding: "12px 16px", textAlign: "left", fontSize: "12px", fontWeight: "600", color: "#94a3b8", borderBottom: "1px solid #334155" }}>DIAS</th>
...
th_new = """                  <th style={{ padding: "12px 16px", textAlign: "left", fontSize: "12px", fontWeight: "600", color: "#94a3b8", borderBottom: "1px solid #334155" }}>DIAS</th>
                  <th style={{ padding: "12px 16px", textAlign: "right", fontSize: "12px", fontWeight: "600", color: "#94a3b8", borderBottom: "1px solid #334155" }}>STATUS</th>
                  ...
                  <th style={{ padding: "12px 16px", textAlign: "right", fontSize: "12px", fontWeight: "600", color: "#94a3b8", borderBottom: "1px solid #334155" }}>KM</th>
                  <th style={{ padding: "12px 16px", textAlign: "right", fontSize: "12px", fontWeight: "600", color: "#94a3b8", borderBottom: "1px solid #334155" }}>TOTAL MEDIÇÃO</th>"""
content = content.replace(th_old, th_new)

# 7. Add columns to Table Body
td_old = """                    <td style={{ padding: "12px 16px", fontSize: "14px", color: "#f8fafc" }}>
...
td_new = """                    <td style={{ padding: "12px 16px", fontSize: "14px", color: "#f8fafc" }}>
                      {item.dias_trabalhados}
                    </td>
...
# 8. Add KM Modal UI at the bottom
modal_old = """      {showGerenciarPostos && (
        ...
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.6)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div style={{ background: "#1e293b", padding: "24px", borderRadius: "12px", width: "100%", maxWidth: "450px", border: "1px solid #334155" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <div>
                <label style={{ display: "block", color: "#94a3b8", fontSize: "12px", marginBottom: "4px" }}>KM Rodado</label>
                <input
...
print("Patched Medicao.jsx successfully")
