...
// 1. State
content = content.replace("  const [isNovoPosto, setIsNovoPosto] = useState(false);",
...
// 2. map mapping
content = content.replace(`const mapaPresencas = new Map();\n    presTrabalhadas.forEach(p => {`,
...
// 4. unmapped
content = content.replace(`          total_colaboradores: presInfo.colaboradores.size,\n          valor_total: 0,\n          diferenca_mensal: 0,\n          detalhes: presInfo.detalhes,\n          _nao_cadastrado: true  // Flag para identificar visualmente\n        });`,
`          total_colaboradores: presInfo.colaboradores.size,
          valor_total: 0,
          diferenca_mensal: 0,
          detalhes: presInfo.detalhes,
...
// 5. Button
content = content.replace(/<button \r?\n              onClick=\{\(\) => setShowGerenciarPostos\(true\)\}/g,
`<button 
              onClick={() => setShowKmModal(true)}
              style={{ padding: "8px 16px", borderRadius: "8px", border: "1px solid #334155", background: "#3b82f6", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px", fontWeight: "500", fontSize: "14px", marginRight: "10px" }}
            >
...
// 6. Table headers (find the exact one for the main table which has >DIAS</th>)
content = content.replace(/<th style=\{\{ padding: .12px 16px., textAlign: .left., fontSize: .12px., fontWeight: .600., color: .#94a3b8., borderBottom: .1px solid #334155. \}\}>DIAS<\/th>\r?\n\s*<th style=\{\{ padding: .12px 16px., textAlign: .right., fontSize: .12px., fontWeight: .600., color: .#94a3b8., borderBottom: .1px solid #334155. \}\}>VALOR MENSAL BASE<\/th>\r?\n\s*<th style=\{\{ padding: .12px 16px., textAlign: .right., fontSize: .12px., fontWeight: .600., color: .#94a3b8., borderBottom: .1px solid #334155. \}\}>TOTAL MEDIÇÃO<\/th>/g,
`<th style={{ padding: "12px 16px", textAlign: "left", fontSize: "12px", fontWeight: "600", color: "#94a3b8", borderBottom: "1px solid #334155" }}>DIAS</th>
...
// 7. Table TD
content = content.replace(/<td style=\{\{ padding: .12px 16px., fontSize: .14px., color: .#f8fafc. \}\}>\r?\n\s*\{item\.dias_trabalhados\}\r?\n\s*<\/td>\r?\n\s*<td style=\{\{ padding: .12px 16px., textAlign: .right., fontSize: .14px., color: .#f8fafc., fontWeight: .500. \}\}>\r?\n\s*\{formatCurrency\(item\.valor_mensal\)\}\r?\n\s*<\/td>\r?\n\s*<td style=\{\{ padding: .12px 16px., textAlign: .right., fontSize: .14px., color: .#10b981., fontWeight: .700. \}\}>\r?\n\s*\{formatCurrency\(item\.valor_total\)\}\r?\n\s*<\/td>/g,
`<td style={{ padding: "12px 16px", fontSize: "14px", color: "#f8fafc" }}>
...
// 8. Modal
content = content.replace(/\{showGerenciarPostos && \(\r?\n\s*<div style=\{\{ position: .fixed., top: 0, left: 0, right: 0, bottom: 0, background: .rgba\(0,0,0,0\.6\)., zIndex: 1000, display: .flex., alignItems: .center., justifyContent: .center., padding: .20px. \}\}>/,
`{showKmModal && (
...
console.log("Patched smoothly via regex JS.");
